...
- `finger_repair.py`: 別候補ページの位置合わせ、手マスクで保護した実画素置換、境界feather。
...
nearest-neighborで戻し、採用ページと同じROI / page contour / split座標へ射影する。
採用ページで手と判定された領域がなければ追加decodeは行わない。

手領域がある場合は、同じstable interval内の別候補をhand overlapの少ない順・page scoreの高い順で
最大5件までdonor候補として元解像度再取得する。各donorも同じページ幾何へ射影した後、
...
指補修は左右ページの射影/分割後、湾曲・照明・白背景などの画質補正より前に行う。
