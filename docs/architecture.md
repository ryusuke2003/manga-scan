...
CLI / Flask loopback Web UI (127.0.0.1:8765)
  └─ ingest → video → motion → candidate sampling → hand + score
       → candidate selection → configured rotation
       → page contour + per-page perspective OR spread perspective + split
       → dewarp → illumination → white normalization / enhancement
       → dedupe / review state → export
...
`interval_gap` 警告を付ける `interval_gap_factor`（デフォルト3.0）とは別のヒューリスティックである。空白の長さから最大4件まで候補時刻を等間隔に推定する。
