...
9. candidate-prefilter safety on annotated same-spread target/donor pairs;
   a frame where the hand position changes must not be collapsed as near-identical
...
The candidate-prefilter check runs even without `--with-hands`. It reuses the
same-spread target/donor annotations, crops to the annotated book ROI, and fails
when a donor moment with different hand placement is incorrectly collapsed with
the occluded target. This guards the speed optimization from deleting the later
clean frame before MediaPipe or finger repair can use it.
