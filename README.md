...
Macで撮影した漫画の動画から、**見開き画像とPDF**を作るローカルアプリです。標準では左右を切らずに保存し、必要な場合は左右ページへの分割も選べます。
...
`output_layout="split"` または新規スキャン画面の「左右のページに分割」で、従来の左右分割処理を使えます。既存プロジェクトは分割出力を維持し、レビュー画面の「見開き・候補フレーム」から見開きごとに切り替えられます。切り替え前の除外状態と左右の並び順は、元の形式へ戻したときに復元します。編集後は「PDFを出力」で反映してください。
