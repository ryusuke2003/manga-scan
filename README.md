...
入力によるメモリ/CPU枯渇を避けるため、デフォルトでは静止画を **60,000,000 pixels / 最大辺12,000px / 1フォルダ5,000枚**、動画を **40,000,000 pixels/frame / 最大辺8,192px / 合計4時間** までに制限します。必要な場合は `config.toml` の `max_image_*` / `max_video_*` を明示的に調整してください。
