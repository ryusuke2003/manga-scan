...
分割出力時のページ別台形補正はデフォルトで `perspective_mode="per_page"` です。従来方式へ固定したい場合は `"spread"` を選べます。この設定は出力形式の `output_layout` とは別です。見開き出力では左右ページの輪郭から外側4点を求めますが、左右を別々には射影変換しません。
