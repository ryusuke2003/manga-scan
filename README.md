...
新規プロジェクトのデフォルトは `output_layout="spread"` です。元フレームの外周を台形補正して、中央で分割せずに保存します。左右別の台形補正・分割位置・湾曲補正は使わず、指補修・照明補正・白背景・グレースケールは引き続き適用します。表紙は従来どおり1ページです。
...
余分な背景や切りすぎた端は、採用中の候補の「切り抜き範囲を調整」で修正できます。元フレームを正しい向きで表示し、指定した四隅はその候補だけに適用します。手動指定した範囲は左右別の自動輪郭検出で上書きしません。切り抜き直しでは、指に隠れた絵や元動画の画面外の絵は復元できません。
...
分割出力時のページ別台形補正はデフォルトで `perspective_mode="per_page"` です。従来方式へ固定したい場合は `"spread"` を選べます。この設定は出力形式の `output_layout` とは別で、見開き出力中は左右別補正を使いません。
