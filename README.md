...
3. **自動判定した向きをプレビューで確認**
   - 新規プロジェクトでは動画の回転メタデータを優先し、無い場合は低解像度3フレームのページ形状から 0 / 90 / 180 / 270° を推定します。
   - 自動判定はプロジェクト作成時の1回だけです。向きに自信がない場合は警告を表示します。
   - 表紙/見開きプレビューが正しければそのまま進み、違う場合だけ0 / 90 / 180 / 270°へ手動変更できます。
4. **表紙を追加する（任意）**
