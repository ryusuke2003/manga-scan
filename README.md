...
ページの下に白・色付き・黒などの表紙が見える場合は、外側の机との境界だけでなく、内側の長い境界も調べます。ただし、表紙が見えない本のページ内に印刷された帯も同じ形に見えるため、色や線だけで自動的に内側を採用しません。初期ROIがすでに内側の境界を支持している場合だけその位置を維持し、判断がつかない場合はページを切らない外側の枠を使ってレビュー画面に確認を促します。表紙の色は判定条件にしていません。
