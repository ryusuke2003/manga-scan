...
def main():
    ...
    legacy_segments, legacy = _recover_missing_segments_high_fps(
        ROOT, manifest, cfg, samples, list(segments), fps, use_batch=False)
    legacy_seconds = time.monotonic() - started
    started = time.monotonic()
    new_segments, current = _recover_missing_segments_high_fps(
        ROOT, manifest, cfg, samples, list(segments), fps, frame_previews=previews)
    grouped_seconds = time.monotonic() - started
    ...
    if args.strict and (report["legacy_recoveries_lost"] or
                        any(old_hits[key] and not new_hits[key] for key in old_hits)):
        raise SystemExit(1)
